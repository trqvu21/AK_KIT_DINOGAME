...
static int32_t current_speed = CACTUS_SPEED_SCALED;

void dino_reset();
// ============================================================
// 2. HÀM HỖ TRỢ: MẠNG (NETWORK HELPERS)
// ============================================================
void rf_mode_rx() {
    nRF24_RXMode(nRF24_RX_PIPE0, nRF24_ENAA_P0, current_rf_channel, 
...
void rf_send_cmd(uint8_t cmd) {
    ...
    nRF24_TXMode(5, 15, current_rf_channel, nRF24_DataRate_1Mbps, 
                 nRF24_TXPower_0dBm, nRF24_CRC_2byte, nRF24_PWR_Up, RF_ADDR, 5);
...
static void handle_network_packets() {
    ...
    if (nRF24_RXPacket(rx_data, 5) == nRF24_RX_PCKT_PIPE0) {
        ...
        if (mp_state == MP_WAITING) {
            ...
            // CƠ CHẾ CÁCH LY (SESSION ISOLATION)
...
// ============================================================
// 3. HÀM HỖ TRỢ: LOGIC GAME (GAMEPLAY HELPERS)
// ============================================================
void dino_reset() {
    ...
    frame_skip = 0; 
    attack_timer = 0;
    current_speed = CACTUS_SPEED_SCALED;
}

static void update_physics_and_speed() {
...
static void update_objects_and_collisions() {
    for (int i = 0; i < 4; i++) {
        if (!objects[i].active) {
...
void dino_update() {
    update_physics_and_speed();
    update_objects_and_collisions();
}

// ============================================================
// 4. HÀM HỖ TRỢ: GIAO DIỆN (VIEW HELPERS)
// ============================================================
...
void view_scr_dino_game() {
    view_render.clear();
    if (mp_state == MP_PLAYING) render_gameplay();
    else render_lobby();
}
...
// ============================================================
// 5. EVENT HANDLER CHÍNH (MAIN TASK)
// ============================================================
void scr_archery_game_handle(ak_msg_t* msg) {
    switch (msg->sig) {
        ...
        case AR_GAME_TIME_TICK: {
            ...
            handle_network_packets(); // Tách mảng xử lý mạng ra cho gọn
            if (mp_state == MP_PLAYING) dino_update(); // Tách mảng tính toán vật lý ra cho gọn
